						  ...
						  <div class="row">
							...
							<div class="col-md-12 mb-40">
								<h2 class="text-center quote-form--header">Health Insurance Plans</h2>
								  <h4 class="text-center">( <?php echo DATE('d F Y', strtotime($date)) ?> / Age <?php echo $age ?> years<?php echo ($age == '0') ? ' '.$ageMonth.' months' : ''; ?> )</h4>					        	
							  </div>
								...
								<style>
									...
									.green-tooltip + .tooltip > .tooltip-inner {background-color: #4caf50; color: #FFF}
								...
								<table class="table table-striped">
									<thead>
										<tr>
											<th>Plan</th>
											<th class="text-center hide-on-mobile">Room</th>
											<th class="text-center hide-on-mobile">IPD</th>
											<th class="text-center hide-on-mobile">OPD</th>
											<th class="text-center hide-on-mobile">Maternity</th>
											<th class="text-center hide-on-mobile">Vision</th>
											<th class="text-center hide-on-mobile">Dental</th>
											<th class="hide-on-mobile">Emergency Expense</th>
											<th class="hide-on-mobile">Annual Limit</th>
											<th>Premium</th>
											<th>&nbsp;</th>
										</tr>
									</thead>
									<tbody>
										...
										<tr>
											<?php foreach( $cols as $col ): ?>
												...
												<?php elseif($col == 'Room'): ?>
													<td class="hide-on-mobile"><?php echo number_format($data[ $col ], 0) ?></td>
												<?php elseif($col == 'Premium'): ?>
													<td style="color: blue; font-size: 22px;">
															<?php 
																if($data[ $col ] == 'Platinum'){
																	$premium_duty = ceil(($plans[0]['price'] * 0.004) + $plans[0]['price']);
																		echo number_format($premium_duty, 0);
																} else if($data[ $col ] == 'Gold'){
																	$premium_duty = ceil(($plans[1]['price'] * 0.004) + $plans[1]['price']);
																	echo number_format($premium_duty, 0);
																} else if($data[ $col ] == 'Silver'){
																	$premium_duty = ceil(($plans[2]['price'] * 0.004) + $plans[2]['price']);
																	echo number_format($premium_duty, 0);
																} else if($data[ $col ] == 'Bronze'){
																	$premium_duty = ceil(($plans[3]['price'] * 0.004) + $plans[3]['price']);
																	echo number_format($premium_duty, 0);
																}
															?>
													</td>
												<?php elseif($col == 'Annuall Limit'): ?>
													<td class="hide-on-mobile"><?php echo $data[ $col ] ?></td>
												<?php else: ?>
														<?php if($data[ $col ] === true): ?>
															<?php
																if($col == 'IPD'){
																	$bubbleText = 'Hospital Expenses : Normal & Customary Charge';
																	if($data['plan'] == 'Bronze'){
																		$bubbleText = 'Hospital Expenses : 200,000';
																	}
																} else if($col == 'OPD'){
																	$bubbleText = 'Normal & Customary Charge';
																	if($data['plan'] == 'Bronze'){
																		$bubbleText = '2,500/Maximum 1 visit per day/30 visits per year';
																	}
																} else if($col == 'Maternity'){
																	$bubbleText = 'Maternity Expenses : 150,000 Surgical Delivery';
																	if($data['plan'] == 'Platinum'){
																		$bubbleText = '90,000 Normal Delivery<br>150,000 Surgical Delivery';
																	} else if ($data['plan'] == 'Gold'){
																		$bubbleText = '70,000 Normal Delivery<br>120,000 Surgical Delivery';
																	} else if($data['plan'] == 'Silver') {
																		$bubbleText = '60,000 Normal Delivery<br>100,000 Surgical Delivery';
																	}
																} else if($col == 'Vision'){
																	$bubbleText = '20,000 (pay80%)';
																} else if($col == 'Dental'){
																	$bubbleText = '80,000 (pay80%)';
																} else if($col == 'Emergency Expense'){
																	$bubbleText = 'Fully Indemnified';
																}
															?>
															<td class="text-center text-primary hide-on-mobile"><i class="fa fa-2x fa-check green-tooltip" data-html="true" data-toggle="tooltip" data-placement="top" title="<?php echo $bubbleText; ?>"></i></td>
														<?php elseif($data[ $col ] === false): ?>
															...
															?>
															<td class="text-center text-danger hide-on-mobile"><i class="fa fa-2x fa-close"></i></td>
															<!-- <td class="text-center text-danger"><i class="fa fa-2x fa-close" data-toggle="tooltip" data-placement="top" title="<?php echo $bubbleText; ?>"></i></td> -->
														<?php else: ?>
														<?php 
															$class = '';
															if($col == 'Vision' || $col == 'Dental'){
																	$class = 'hide-on-mobile';
																}
															?>
															<td class="text-center <?php echo $class; ?>"><?php echo $data[ $col ] ?></td>
														<?php endif; ?>
												<?php endif; ?>
												<?php endforeach; ?>
...
one time contribution to incurred medical costs up to the selected amount)<br/>
										  ...
										  <div class="rowFluid">
											...
											<div class="col-xs-12 col-sm-6">
												 <a href="http://thaivisaprotect.com/contact.html" target="_blank" class="btn btn-block btn-default btn-contact">Contact us >></a>
											</div>
										</div>
