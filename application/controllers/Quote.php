...
class Quote extends Public_Controller {
    ...
    function doQuote(){
        ...
        if( $save ){
            ...
            $this->email->from('onlinequote@pacificcrosshealth.com', 'PCHI');
            $this->email->to( $this->input->post('email', true) );
            $this->email->bcc(array('onlinequote@th.pacificcrosshealth.com'));
            // $this->email->bcc(array('Chotiwit.ks@gmail.com', 'bbird_ss@outlook.com', 'jonathan@thaivisa.com'));

            $this->email->subject('ขอบคุณที่เช็คเบี้ยประกันสุขภาพกับ แปซิฟิค ครอส');
            ...
            $mail = $this->email->send();
                $this->session->set_flashdata('quote_success', 'PCHI');
            if( !$mail ){
                $error = true;
